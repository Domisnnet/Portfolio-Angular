...
export class CosmicLayerService {
  ...
  readonly region = computed<CosmicRegion>(() => {
    switch (this.layer()) {
      case 'projects':
        return COSMIC_REGIONS[CosmicRegionId.Featured];
      case 'deep-space':
        return COSMIC_REGIONS[CosmicRegionId.DeepSpace];
      case 'stable-orbit':
        return COSMIC_REGIONS[CosmicRegionId.StableOrbit];
      case 'unstable-orbit':
        return COSMIC_REGIONS[CosmicRegionId.UnstableOrbit];
      case 'wormhole':
        return COSMIC_REGIONS[CosmicRegionId.Featured];
      default:
        return COSMIC_REGIONS[CosmicRegionId.Featured];
  ...
  advance(): JumpStage {
    ...
    switch (progress) {
      case 1:
        this.jumpStageSignal.set('charging');
        break;
      case 2:
        this.jumpStageSignal.set('critical');
        break;
      case 3:
        this.jumpStageSignal.set('wormhole');
  ...
  is(layer: CosmicLayer): boolean {
    return this.layer() === layer;
  }
  isStage(stage: JumpStage): boolean {
    return this.jumpStage() === stage;
  }
  isProjects(): boolean {
    return this.is('projects');
  }
  isWormhole(): boolean {
    return this.is('wormhole');
  }
  isDeepSpace(): boolean {
    return this.is('deep-space');
  }
  isIdle(): boolean {
    return this.isStage('idle');
  }
  isCharging(): boolean {
    return this.isStage('charging');
  }
  isCritical(): boolean {
    return this.isStage('critical');
  }
  isJumping(): boolean {
    return this.isStage('wormhole');
  }
}
